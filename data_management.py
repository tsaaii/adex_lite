    ...
    def create_pdf_report(self, records_data, save_path):
        ...
        try:
            ...
            for i, record in enumerate(records_data):
                ...
                print_date = record.get('second_timestamp')
                ...
                vehicle_data = [
                    ...
                    Paragraph("<b>Time:</b>", label_style), Paragraph(record.get('second_timestamp', record.get('time', '')).split()[1] if record.get('second_timestamp') else record.get('time', ''), value_style)],
...
# Weighment Information
                ...
                # 4-Image Grid Section - FIXED WITH PROPER WATERMARKS
                elements.append(Paragraph("VEHICLE IMAGES (4-Image System)", section_header_style))
                
                # Get data for watermarks
                site_name = record.get('site_name', 'Unknown')
                vehicle_no = record.get('vehicle_no', 'Unknown')
                # NEW CODE (correct):
                first_timestamp = record.get('first_timestamp', '')  # From CSV column 9
                second_timestamp = record.get('second_timestamp', '')  # From CSV column 11
                
                # Create proper watermark text for each image
                # Format: "Site - Vehicle - Timestamp - Description"
                image_paths = [
                    (record.get('first_front_image', ''), 
                     f"{site_name} - {vehicle_no} - {first_timestamp} - 1ST FRONT",
                     ticket_no),
                    (record.get('first_back_image', ''), 
                     f"{site_name} - {vehicle_no} - {first_timestamp} - 1ST BACK",
                     ticket_no),
                    (record.get('second_front_image', ''), 
                     f"{site_name} - {vehicle_no} - {second_timestamp} - 2ND FRONT",
                     ticket_no),
                    (record.get('second_back_image', ''), 
                     f"{site_name} - {vehicle_no} - {second_timestamp} - 2ND BACK",
                     ticket_no)
                ]
                ...
                # Process all 4 images with watermarks
                processed_images = []
                
                for img_filename, watermark_text, ticket_id in image_paths:
                    if img_filename and img_filename.strip():
                        ...
                        if os.path.exists(img_path):
                            try:
                                # Pass both watermark text and ticket_id
                                temp_img_path = self.prepare_image_for_pdf(img_path, watermark_text, ticket_id)
                                if temp_img_path and os.path.exists(temp_img_path):
                                    ...
                                    self.logger.debug(f"Successfully processed image with watermark: {img_filename}")
                                else:
    ...
    def prepare_image_for_pdf(self, image_path, watermark_text, ticket_id=None):
        """Prepare image for PDF with watermark using your existing add_watermark function
        
        Args:
            image_path: Path to the image file
            watermark_text: Text to add as watermark (format: "Site - Vehicle - Timestamp - Description")
            ticket_id: Optional ticket ID to add at bottom
        """
        try:
            import cv2
            import os
            ...
            # Apply watermark using your existing function
            ...
            # Parse the watermark text
            if watermark_text:
                parts = [part.strip() for part in watermark_text.split(' - ')]
                ...
                if len(parts) >= 4:
                    ...
                    cv2.rectangle(overlay, (0, 0), (max_text_width + 20, overlay_y_end), (0, 0, 0), -1)
            ...
            if ticket_id:
                ...
                (ticket_width, ticket_height), _ = cv2.getTextSize(ticket_text, font, font_scale, thickness)
                
                # Create overlay background for ticket at BOTTOM
                overlay_ticket = result.copy()
                overlay_y_start = height - ticket_height - 20
                cv2.rectangle(overlay_ticket, (0, overlay_y_start), (ticket_width + 20, height), (0, 0, 0), -1)
            ...
            temp_filename = f"temp_pdf_image_{timestamp_file}.jpg"
            ...
            success = cv2.imwrite(temp_path, result, [cv2.IMWRITE_JPEG_QUALITY, 95])
            ...
            if not os.path.exists(temp_path) or os.path.getsize(temp_path) == 0:
                self.logger.error(f"Temporary image issue: {temp_path}")
                return None
            
            self.logger.debug(f"Successfully prepared image with watermark: {temp_path}")
            return temp_path
