...
class WeighbridgeManager:
    """Optimized weighbridge manager with buffering approach from working test script"""
    ...
    def _optimized_read_loop(self):
        """FIXED: Reading loop using buffering approach from working test script"""
        
        # Buffer to accumulate complete messages (same as working test script)
        buffer = ""
        # Pattern to match complete weight values (5 digits like 10990)
        weight_pattern = re.compile(r'(\d{5})')
            ...
            try:
                ...
                if self.serial_connection and self.serial_connection.is_open:
                    if self.serial_connection.in_waiting > 0:
                        try:
                            # Read and decode with UTF-8 (same as working test script)
                            data = self.serial_connection.read(self.serial_connection.in_waiting).decode('utf-8', errors='ignore')
                            
                            if data:
                                # Add to buffer (same as working test script)
                                buffer += data
                                print(f"DEBUG: Received data: '{data}', Buffer now: '{buffer}'")
                                
                                # Look for complete weight values in the buffer (same as working test script)
                                weight_match = weight_pattern.search(buffer)
                                if weight_match:
                                    weight_value = weight_match.group(1)
                                    print(f"DEBUG: Found weight pattern: {weight_value}")
                                    
                                    # Convert to float for processing
                                    weight = float(weight_value)
                                    print(f"WEIGHT: {weight_value} (parsed as {weight:.2f} kg)")
                                    
                                    # Remove the processed data from buffer (same as working test script)
                                    buffer = buffer[weight_match.end():]
                                ...
                                # Reset buffer if it gets too full to prevent memory issues
                                if len(buffer) > 50:
                                    buffer = buffer[-25:]  # Keep last 25 characters
                ...
                time.sleep(0.01)  # 10ms delay (same as working test script)
    ...
    def _process_weight(self, weight):
        ...
        try:
            ...
            if self.stable_count >= self.stable_readings_required:
                print(f"WEIGHT: {weight:.2f} kg (stable reading)")
